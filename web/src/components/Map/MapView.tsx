...
//   audioUrl?: string;
//   categoryId?: string;
//   // Optional latitude and longitude for compatibility with existing code
//   lat?: number;
//   lng?: number;
...
//           audioUrl: getString(p.audioUrl),
//           // location fields
//           location: {
//             latitude: p.location?.latitude ?? p.lat ?? (p.location ? p.location.lat : undefined) ?? 0,
//             longitude: p.location?.longitude ?? p.lng ?? (p.location ? p.location.lng : undefined) ?? 0,
//           },
...
//           // Fetch detailed POI with translation and TTS based on user language
//           const language = useUserStore.getState().language || 'vi';
//           try {
//             const res = await getPoi(nearbyPoi.id, language);
//             if (res.success && res.data) {
//               const p = res.data as any;
//               const detailedPoi: POI = {
//                 id: p.id ?? nearbyPoi.id,
//                 lat: p.location?.latitude ?? nearbyPoi.location?.latitude ?? nearbyPoi.lat,
//                 lng: p.location?.longitude ?? nearbyPoi.location?.longitude ?? nearbyPoi.lng,
//                 rating: p.rating ?? nearbyPoi.rating,
//                 distance: nearbyPoi.distance,
//                 title: getString(p.title ?? p.name, language as any),
//                 summary: getString(p.summary ?? p.description, language as any),
//                 audioUrl: getString(p.audioUrl ?? p.audioUrls ?? p.audioUrl, language as any) || getString(p.audioUrl, 'vi'),
//                 location: {
//                   latitude: p.location?.latitude ?? nearbyPoi.location?.latitude ?? nearbyPoi.lat,
//                   longitude: p.location?.longitude ?? nearbyPoi.location?.longitude ?? nearbyPoi.lng,
//                 },
//                 // preserve other optional fields if needed
//                 address: p.address,
//                 contact: p.contact || nearbyPoi.contact || undefined,
//                 categoryId: p.categoryId,
//                 status: p.status,
//                 visibility: p.visibility,
//                 isActive: p.isActive,
...
//               <div className="p-1">
//                 <h3 className="font-bold text-orange-600">{poi.title}</h3>
    ...
    <div className="p-1 min-w-[160px]">
      ...
      {poi.distance !== undefined && (
        <div className="mt-1 flex items-center gap-1 text-xs text-zinc-500">
          <MapPin size={10} />
          <span>Cách {Math.round(poi.distance)}m</span>
        </div>
      )}
      <button
        onClick={handlePlay}
